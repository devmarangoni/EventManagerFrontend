...
export function FloatingBalloons() {
  ...
  const initBalloons = (width: number, height: number) => {
    ...
    for (let i = 0; i < balloonCount; i++) {
      newBalloons.push({
        ...
        stringLength: Math.random() * 40 + 60, // Longer strings (60-100px)
        stringWave: Math.random() * Math.PI * 2, // Random starting phase
        stringSpeed: Math.random() * 0.01 + 0.005, // Slower string animation
        opacity: Math.random() * 0.3 + 0.7,
        sway: 0,
        swaySpeed: Math.random() * 0.001 + 0.0005, // Slower sway
      })
    }
  ...
  const drawBalloon = (ctx: CanvasRenderingContext2D, balloon: Balloon, time: number) => {
    ...
    // Draw string with wave - slower and longer
    ctx.beginPath()
    ctx.moveTo(0, balloon.size * 0.6)

    // Animated string with gentler wave
  ...
  return (
    <canvas
      ...
      style={{
        zIndex: 1, // Changed from -1 to 1 to be above backgrounds but below content
        opacity: 0.8,
      }}
    />
  )
